...
export default function DeleteRecordsMongoose(){
    usePrismHighlightAll(); 
    let SubIcon =
    "https://cdn.iconscout.com/icon/premium/png-256-thumb/delete-1432400-1211078.png";
    return (
        <>
            <LoginCheck />
        <MainHeader />
  ...
  <div className="template">
    <div className="top-title">
      <span>DELETE DATA</span><img src={SubIcon} alt={SubIcon} id="b"/>
    </div>

    <div className="allData">
<h1>FOLLOW THE STEP TO DELETE DATA IN MONGOOSE USING BACKEND</h1>
            <h2>Router or auth file.js</h2>

            <pre>
                <code className="language-js">
                    {`// ! delete notes
router.delete('/deleteNote/:id',VerfifyUser, async(req,res)=>{
try {
            // find note to update
            //usernote get from schema
    let deleteNote=await userNotes.findById(req.params.id);//params get from parameter in url
    if(!deleteNote){
    ...
    // create new 
    deleteNote=await userNotes.findByIdAndDelete(req.params.id)
    res.send({"success":"note successfully delete",deleteNote: deleteNote})
